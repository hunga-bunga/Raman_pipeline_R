...
##############################################################################
# ==============================================================================
#   PIPELINE COMPLETO: DATASET FT-RAMAN SrE.IR (100% AS TUAS FUNÇÕES)
# ==============================================================================

if (!requireNamespace("caret", quietly = TRUE)) install.packages("caret")
if (!requireNamespace("ChemoSpec", quietly = TRUE)) install.packages("ChemoSpec")
library(caret)
library(ChemoSpec)

# ------------------------------------------------------------------------------
# 1. CARREGAMENTO E CONFIGURAÇÃO DE SEGURANÇA (CONSERVAÇÃO TOTAL DE CLASSES)
# ------------------------------------------------------------------------------
cat("==================================================================\n")
cat("    1. CARREGAMENTO DO DATASET FT-RAMAN DO CHEMOSPEC\n")
cat("==================================================================\n")

data(SrE.IR)

# Estruturação inicial síncrona
df_bruto <- list(
  data = SrE.IR$data,                          
  wavenumbers = SrE.IR$freq,                   
  y_train = as.factor(SrE.IR$groups),          
  Patient_ID = as.factor(paste0("Lote_", rep(1:(nrow(SrE.IR$data)/5 + 1), each=5)[1:nrow(SrE.IR$data)]))
)

# Filtro estrito para remover NAs estruturais
indices_validos <- which(!is.na(df_bruto$y_train))
X_inicial <- df_bruto$data[indices_validos, , drop = FALSE]
y_inicial <- df_bruto$y_train[indices_validos]
id_inicial <- df_bruto$Patient_ID[indices_validos]

# RECOMPOSIÇÃO SINTÉTICA (Oversampling Manual para Folds do Caret):
# Garante que nenhuma classe tem menos de 5 amostras para que o teu ML_raman funcione
contagem <- table(y_inicial)
classes_criticas <- names(contagem[contagem < 5])

X_expandido <- X_inicial
y_expandido <- y_inicial
id_expandido <- id_inicial

for (cl in classes_criticas) {
  idx_cl <- which(y_inicial == cl)
  repeticoes <- 5 - length(idx_cl)
  
  # Duplica as amostras raras para preencher os Folds vazios
  idx_replicar <- rep(idx_cl, length.out = repeticoes)
  
  X_expandido <- rbind(X_expandido, X_inicial[idx_replicar, , drop = FALSE])
  y_expandido <- c(y_expandido, y_inicial[idx_replicar])
  id_expandido <- c(id_expandido, id_inicial[idx_replicar])
}

df_clinico <- list(
  data = X_expandido,
  wavenumbers = df_bruto$wavenumbers,
  y_train = as.factor(y_expandido),
  Patient_ID = as.factor(id_expandido)
)

cat("Distribuição final após balanceamento por replicação (0% classes removidas):\n")
print(table(df_clinico$y_train))
cat("\nTotal de amostras enviadas para as tuas funções:", nrow(df_clinico$data), "\n\n")


# ------------------------------------------------------------------------------
# 2. FLUXO SEQUENCIAL DE PRÉ-PROCESSAMENTO (TUAS FUNÇÕES)
# ------------------------------------------------------------------------------
cat("--- A EXECUTAR PRÉ-PROCESSAMENTO (TUAS FUNÇÕES) ---\n")
...
min_wn_sre <- min(df_clinico$wavenumbers)
max_wn_sre <- max(df_clinico$wavenumbers)
df_cropped <- crop_espetro(df_clinico, min_wn = min_wn_sre, max_wn = max_wn_sre)
...
df_minmax$Patient_ID <- df_clinico$Patient_ID
df_l2$Patient_ID     <- df_clinico$Patient_ID


# ------------------------------------------------------------------------------
# 3. EXTRAÇÃO E TRANSFORMAÇÕES MATEMÁTICAS ADICIONAIS (TUAS FUNÇÕES)
# ------------------------------------------------------------------------------
...
plot(df_minmax$wavenumbers, colMeans(df_minmax$data), type="l", col="blue", lwd=1.5,
     main="Tua Normalização Min-Max (Média SrE.IR)", xlab="cm-1", ylab="Intensidade")
plot(df_l2$wavenumbers, colMeans(df_l2$data), type="l", col="darkgreen", lwd=1.5,
     main="Tua Normalização Vetorial L2 (Média SrE.IR)", xlab="cm-1", ylab="Intensidade")
par(mfrow = c(1, 1))
...
# ------------------------------------------------------------------------------
# 5. MOCK EXTRA: EXTRAÇÃO DE FEATURES DE PICOS (TUAS FUNÇÕES)
# ------------------------------------------------------------------------------
cat("\n--- A EXTRAIR FEATURES ESTRUTURAIS DE PICOS ---\n")
nomes_falsos_espectros <- paste0(df_l2$y_train, "$sample_", 1:nrow(df_l2$data))
picos_alvo <- c(1750, 2900, 3400) 

lista_propriedades_mock <- lapply(1:nrow(df_l2$data), function(i) {
  list(
    Peaks = picos_alvo,
    peak_heights = c(df_l2$data[i, which.min(abs(df_l2$wavenumbers - 1750))], 
                     df_l2$data[i, which.min(abs(df_l2$wavenumbers - 2900))], 
                     df_l2$data[i, which.min(abs(df_l2$wavenumbers - 3400))]),
    prominences = c(0.5, 0.8, 0.6), widths = c(10, 15, 12), width_heights = c(5, 7, 6),
    left_minima = c(1730, 2870, 3360), right_minima = c(1770, 2930, 3440), AUC = c(5, 12, 8)
  )
})
...
# ------------------------------------------------------------------------------
# 6. CAPTURA E TREINO GLOBAL ATRAVÉS DAS TUAS FUNÇÕES DE MODELAÇÃO NATIVAS
# ------------------------------------------------------------------------------
cat("==================================================================\n")
cat("    6. EXECUÇÃO DO TREINO VIA TUAS FUNÇÕES ORIGINAIS (ML_raman)\n")
cat("==================================================================\n")
...
cat("Accuracy final obtida nos dados de teste:", round(resultado_cnn$test_accuracy * 100, 2), "%\n\n")
...
visualizar_raw_vs_pre(df_raw = SrE.IR, df_processado = df_l2)
