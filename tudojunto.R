...
airPLS_fast_dataset <- function(dataset, lambda = 1e5, max_iter = 50) {
  ...
  for (j in 1:N_amostras) {
    ...
    #Subtrai o mínimo individual para forçar o espetro a começar no 0
...
raman_pca_plot <- function(dataset) {
  ...
  # Definir cores bonitas para as 5 classes
...
ML_raman <- function(dataset, method = "rf", train_prop = 0.7) {
  ...
  # Usa 5-fold CV para validar o modelo internamente
...
raman_radar_plot <- function(dataset, sample_idx = 1) {
  ...
  # Faz múltiplas camadas transparentes para dar o efeito de brilho (glow)
...
visualizar_media_espetros <- function(df_raw, df_processado, metodo_norm = "L2") {
  ...
  if (toupper(metodo_norm) == "L2") {
    ...
    # Fallback genérico caso outro nome seja passado
    cor_linha  <- "black"
...
# ==============================================================================
#   PIPELINE ABSOLUTO: CARREGAMENTO .NPY, PRÉ-PROCESSAMENTO E TESTES OTIMIZADOS
# ==============================================================================
...
cat("==================================================================\n")
cat("   1. CARREGAMENTO DOS FICHEIROS MATRIZ (.npy) DE RAIZ\n")
cat("==================================================================\n")

# Carregamento inicial a partir de ficheiros locais
df <- dataset_npy(
...
# ------------------------------------------------------------------------------
# 2. FILTRAGEM CRÍTICA ANTI-NA NAS CLASSES DO DATASET CLINICO
# ------------------------------------------------------------------------------
cat("\n[Filtro] A remover amostras sem classificação biológica (NAs)...\n")
indices_validos <- which(!is.na(df$y_train))

# Construção da estrutura limpa e síncrona para o pipeline
df_clinico <- list(
  data = df$data[indices_validos, , drop = FALSE],
  wavenumbers = df$wavenumbers,
  y_train = as.factor(df$y_train[indices_validos])
)

cat("Amostras brutas lidas dos ficheiros:", nrow(df$data), "\n")
cat("Amostras ativas para modelação (sem NAs):", nrow(df_clinico$data), "\n\n")


# ------------------------------------------------------------------------------
# 3. FLUXO SEQUENCIAL COMPLETO DE PRÉ-PROCESSAMENTO (TUAS FUNÇÕES)
# ------------------------------------------------------------------------------
cat("--- A EXECUTAR FLUXO DE PRÉ-PROCESSAMENTO ---\n")

# A. Alinhamento de Picos (Exemplo na Amostra 1)
alinhamento_teste <- align_silicon_peak(
  wavenumber = df_clinico$wavenumbers, 
  intensity = df_clinico$data[1, ], 
  target_peak = 1003, 
  search_window = 15
)

# B. Recorte Espectral (Crop) para a Fingerprint Region
cat("--> Cortando região espectral (500 a 1700 cm-1)...\n")
df_cropped <- crop_espetro(df_clinico, min_wn = 500, max_wn = 1700)

# C. Remoção de Raios Cósmicos (Despiking)
cat("--> Removendo picos de ruído cósmico...\n")
df_despiked <- dataset_despike(df_cropped, ma = 10, threshold = 3.5)

# D. Suavização via Savitzky-Golay (Denoising)
cat("--> Executando filtro Savitzky-Golay (Janela 15)...\n")
df_sg <- savitzky_golay(df_despiked, p.order = 2, window = 15, deriv = 0)

# E. Correção de Baseline (Remoção de Fluorescência via ALS)
cat("--> Corrigindo linha de base ALS...\n")
df_baseline <- baseline_correction(df_sg, method = "als")

# F. Geração dos dois Extremos de Normalização (L2 vs Min-Max)
cat("--> Gerando matrizes normalizadas L2 e Min-Max...\n")
df_minmax <- raman_norm(df_baseline, method = "min-max")
df_l2     <- raman_norm(df_baseline, method = "L2")


# ------------------------------------------------------------------------------
# 4. DIAGNÓSTICO VISUAL COMPLETO DO DATASET
# ------------------------------------------------------------------------------
cat("\n--- A GERAR GRÁFICOS DE DIAGNÓSTICO ---\n")

# Gráfico A: Contraste de Normalizações
par(mfrow = c(2, 1))
plot(df_minmax$wavenumbers, colMeans(df_minmax$data), type="l", col="blue", lwd=1.5,
     main="Tua Normalização Min-Max (Média Dataset Clínico)", xlab="cm-1", ylab="Intensidade")
plot(df_l2$wavenumbers, colMeans(df_l2$data), type="l", col="darkgreen", lwd=1.5,
     main="Tua Normalização Vetorial L2 (Média Dataset Clínico)", xlab="cm-1", ylab="Intensidade")
par(mfrow = c(1, 1))

# Gráfico B: Projeções Radar Individuais e de Classe
df_radar_indiv1 <- raman_radar_plot(df_baseline, sample_idx = 1)
df_radar_indiv5 <- raman_radar_plot(df_baseline, sample_idx = 4)
raman_radar_class_averages(df_l2)

# Gráfico C: Análise de Agrupamento Espacial (PCA Plot)
meu_pca_res <- raman_pca_plot(df_l2)


# ------------------------------------------------------------------------------
# 5. BLOCO DE TESTES DE MACHINE LEARNING (RF & SVM)
# ------------------------------------------------------------------------------
cat("\n==================================================================\n")
cat("   5. AVALIAÇÃO DE MODELOS PREDICTIVOS CLÁSSICOS (L2 vs MIN-MAX)\n")
cat("==================================================================\n")

# --- MODELO A: RANDOM FOREST (DADOS L2) ---
cat("\n>>> Executando: Random Forest (L2) ...\n")
...
if (!is.null(rf_l2_vis$value)) {
  ...
  cat("\nMatriz de Confusão Acumulada nos Folds (RF L2):\n")
  print(rf_l2_vis$value$resampledCM)
}

# --- MODELO B: SVM RADIAL (DADOS L2) ---
cat("\n>>> Executando: SVM Radial (L2) ...\n")
...
if (!is.null(svm_l2_vis$value)) {
  ...
  cat("\nMatriz de Confusão Acumulada nos Folds (SVM L2):\n")
  print(svm_l2_vis$value$resampledCM)
  
  # Gráfico de Importância de Wavenumbers para o SVM
  importancia <- varImp(svm_l2_vis$value)
  plot(importancia, top = 20, main = "Top 20 Wavenumbers Críticos (SVM L2)")
}

# --- MODELO C: RANDOM FOREST (DADOS MIN-MAX) ---
cat("\n>>> Executando: Random Forest (Min-Max) ...\n")
...
# --- MODELO D: SVM RADIAL (DADOS MIN-MAX) ---
cat("\n>>> Executando: Support Vector Machine (Min-Max) ...\n")
...
# ------------------------------------------------------------------------------
# 6. TESTE DEEP LEARNING: CNN 1D OTIMIZADA PARA EVITAR UNDERFITTING
# ------------------------------------------------------------------------------
cat("\n==================================================\n")
cat("   6. TREINO DA REDE NEURONAL CONVOLUCIONAL (CNN 1D)\n")
cat("==================================================\n")



cat("A iniciar treino longo com 50 épocas para convergência de curvas...\n")
...
# Imprime as novas métricas de validação
cat("\n[RESULTADOS FINAIS - CNN 1D DEEP LEARNING]:\n")
cat("Accuracy obtida nos dados de teste:", round(resultado_cnn$test_accuracy * 100, 2), "%\n\n")

cat("Matriz de Confusão Detalhada (Análise de Sensibilidade por Classe):\n")
print(resultado_cnn$confusion_matrix)

# Exibe
plot(resultado_cnn$history)

# Executar a nova visualização estatística no teu dataset SRE
visualizar_raw_vs_pre(df_raw = df, df_processado = df_l2)
